...
class AbstractCredentialIssuer(metaclass=abc.ABCMeta):
    ...
    def issue_credential(
        self, credential, username, status=UserCredentialStatus.AWARDED, attributes=None, date_override=None, request=None
    ):
        """
    ...
    def set_credential_date_override(self, user_credential, date_override=None):
        ...
        if date_override.get("date"):
            UserCredentialDateOverride.objects.update_or_create(
                user_credential=user_credential,
                defaults={"date": date_override.get("date")}
            )
        else:
            UserCredentialDateOverride.objects.filter(
                user_credential=user_credential
            ).delete()
